...
class Detector(ABC):
    ...
    def create_input_file(self, source, file_name, full_detector=True, electrontrack=False):
        ...
        """
        string = f'C  !Energy = {source.energy}\nC  !Position = {source.position()}\nC  !Weight = {source.weight(self, full_detector)}\nC  !Type = {source.type}\n'
        string += 'C  **************************************************************\nC    CELLS\nC  **************************************************************\n'
        string += ''.join(self.cells(source, electrontrack))
        string += ''.join(source.cells(self, electrontrack))
        string += ''.join(self.world_cells(source, electrontrack))
        string += 'C    ************************************************************\n\nC    ************************************************************\n'
        string += 'C    SURFACES\nC    ************************************************************\n'
        string += ''.join(self.surfaces())
        string += ''.join(source.surfaces())
        string += ''.join(self.world_surfaces(source))
        string += 'C    ************************************************************\n\nC    SOURCE\n'
        string += 'C    ************************************************************\n'
        string += ''.join(self.mode(electrontrack))
        string += ''.join(source.source(self, full_detector))
        string += 'C    ************************************************************\n'
        string += ''.join(source.transformation())
        string += 'C    ************************************************************\nC    MATERIAL\nC    ************************************************************\n'
        string += ''.join(self.materials(source))
        string += 'F8:P 40\nC  **  **  **  **  **  **  **\nC    FIRST THREE ENERGIES SHOULD BE 0 1.E-6 AND 1.E-3\n'
        string += 'C    THEN SHOULD HAVE E-0.00199 3I E+0.00201\nC  **  **  **  **  **  **  **\n'
        string += ''.join(self.tally_energy(source.energy))
        string += 'C   TF8  6J 6\nDBCN  7J 123456\n'
        string += ''.join(source.lost_card())
        string += f'NPS  {source.nps}\n'
...
class MCNPMaterialLibrary:
    def __init__(self):
        ...
        """
        self.materials = {'c':MCNPMaterial(1, 2.62, 'C:100%[C]', 'C'),
                          'be':MCNPMaterial(2, 1.85, 'BE:100%[BE]', 'BE'),
                          'air':MCNPMaterial(3, 0.0012, 'N:78.50%O:21.1%AR:0.40%[Air]', 'DRYAIR'),
                          'al':MCNPMaterial(4, 2.7, 'Al:100%[Al]', 'Al'),
                          'ge':MCNPMaterial(5, 5.35, 'GE:100%[GE]', 'GE'),
                          'pb':MCNPMaterial(6, 11.4, 'PB:100%[PB]', 'PB'),
                          'mg':MCNPMaterial(7, 1.74, 'MG:100%[MG]', 'MG'),
                          'water':MCNPMaterial(8, 1.0, 'H:11.19%O:88.81%[WATER]', 'WATER'),
                          'delrin':MCNPMaterial(9, 1.45, 'H:6.71%C:40.00%O:53.28%[DELRIN]', 'DELRIN'),
                          'cu':MCNPMaterial(10, 8.96, 'CU:100%[CU]', 'CU'),
                          'teflon':MCNPMaterial(11, 2.1, 'C:33.33%F:66.67%[Teflon]'),
                          'mylar':MCNPMaterial(12, 1.4, 'H:40.91%C:36.36%O:22.73%[Mylar]'),
                          'w':MCNPMaterial(13, 17.0, 'W:100%[W]', 'W'),
                          'polycarb':MCNPMaterial(14, 1.17, 'H:7.06%C:78.92%O:14.02%[POLYCARB]', 'POLYCARB'),
                          'glassfiber':MCNPMaterial(15, 0.23, 'O:37.10%NA:9.42%SI:26.74%CA:26.74%[GLASSFIB]', ''),
                          'ss':MCNPMaterial(16, 7.8, 'C:0.08%CR:19.00%FE:71.92%NI:9.00%[304SS]', '304SS'),
                          'kapton':MCNPMaterial(17, 2.1, 'H:4.81%C:57.75%N:7.49%O:29.95%[Kapton]'),
                          'porousglass':MCNPMaterial(18, 1.85, 'B:0.93%O:53.72%SI:45.35%[PORGLASS]', 'PORGLASS'),
                          'ceramic':MCNPMaterial(22, 2.0, 'B:43.56%N:56.44%[CERAMIC]', '[CERAMIC]'),
                          'in':MCNPMaterial(23, 7.31, 'IN:100%[IN]', 'IN'),
                          'ti':MCNPMaterial(24, 4.506, 'TI:100%[TI]', 'TI'),
